...
class LRmulticlass(object):
		...
		def xvector(self, jsonInstance):
				fsvoc = open("vocab_train.save", 'rb')
				self.vocab = pickle.load(fsvoc)
				self.vocab.Lock()
				#print self.vocab.GetVocabSize()
				return self.json2Vector(jsonInstance).reshape(1,-1)
...
for event in ['elections', 'oscar_ba', 'oscar_bas', 'oscar_bp']:
		...
		for tweet in iterator:
			...
			if not 'text' in ptweet:
				...
				tmp = lrModel.xvector(tweetFeatures)
				if xmatInit == False:
						xmat = tmp
						xmatInit = True
				else:
						xmat = np.vstack((xmat, tmp))
				idfile.write(str(ptweet['id']) + "\n")
